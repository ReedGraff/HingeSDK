...
class HingeTools:
    ...
    def __init__(self, api_client: HingeAPIClient, media_client: HingeMediaClient):
        """
    ...
    def download_recommendation_content(
        self,
        ...
        try:
            ...
            recommendations = self.api_client.get_recommendations(
                active_today=active_today, new_here=new_here
            )
    ...
    def _download_user_images(
        self, user_id: str, profile: Dict, output_path: str
    ) -> None:
        """
        ...
        for idx, photo in enumerate(photos):
            ...
            try:
                ...
                self.logger.error(
                    f"Failed to download image {cdn_id} for user {user_id}: {str(e)}"
                )
    ...
    def create_profile_json(
        ...
        active_today: bool = False,
        new_here: bool = False,
        output_file: str = "hinge_profiles.json",
    ) -> None:
        """
        ...
        try:
            ...
            mapping_path = os.path.join(
                os.path.dirname(__file__), "assets/prompts.json"
            )
            if not os.path.exists(mapping_path):
                self.logger.error(f"Question mapping file not found: {mapping_path}")
                raise FileNotFoundError(
                    f"Question mapping file not found: {mapping_path}"
                )
            ...
            # Get profiles
            self.logger.info(
                f"Fetching profiles for {len(user_ids)} users from {source.value}..."
            )
            profiles = self.api_client.get_public_users(user_ids)
    ...
    def scrape_recommendations_multiple(
        self,
        iterations: int = 40,
            ...
            for i in range(iterations):
                ...
                for profile in profiles:
                    ...
                    for answer in profile_data.get("answers", []):
                        ...
                        if prompt_data["type"] == "voice":
                            transcription = answer.get("transcription", {})
                            prompt_data["response"] = transcription.get(
                                "transcript", ""
                            )
                            prompt_data["voice_url"] = answer.get("url")
                            prompt_data["waveform"] = answer.get("waveform")
                ...
                # Log results
                self.logger.info(
                    f"Iteration {i + 1}: Added {new_profiles} unique profiles, skipped {duplicate_profiles} duplicates. Total profiles: {len(existing_data)}"
                )
                ...
                if i < iterations - 1:
                    ...
                    self.logger.info(
                        f"Sleeping for {sleep_time:.2f} seconds before next scrape..."
                    )
                    time.sleep(sleep_time)

            self.logger.info(
                f"Completed {iterations} scrapes. Final data saved to {output_path} with {len(existing_data)} unique profiles."
            )
